...
trait EnrichedTrees extends TracingImpl {
  ...
  object ApplyExtractor {
    ...
    def unapply(t: Apply): Option[(Tree, List[Tree])] = extract(t) \\ { res =>
      trace(s"ApplyExtractor: Extracted $res from $t")
    }
    ...
    private def tryParseMethodCallWithouRanges(fun: Tree, args: List[Tree], end: Int): Option[(Tree, List[Tree])] = sortByPosition(args).headOption.map(_.pos) match {
    ...
    private def tryPositionMarkerAtArgListStart(funPos: Position, firstArgPos: OffsetPosition): Option[SourceWithMarker] = {
      ...
      val funPreamble = Movements.id ~ commentsAndSpaces ~ '('
  ...
  object BlockExtractor {
    private def findParamAssignment(argsSource: String, paramName: String): Option[Int] = {
      val mvnt = until(paramName ~ spaces ~ '=', skipping = skipWhileScanningArgsList)
    ...
    def unapply(t: Block) = {
      ...
      removeNewPatternMatchingCruft(t) match {
        case t: Block =>
